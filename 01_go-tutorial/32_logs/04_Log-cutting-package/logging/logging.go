...
type TimeWriter struct {
	Dir        string //日志目录
	Compress   bool   //是否采用压缩算法
	ReserveDay uint   //日志最多保存个数

	curFilename string   //当前写入日志的日志文件名称
	file        *os.File //日志文件句柄
	mu          sync.Mutex
	startMill   sync.Once
	millCh      chan bool //协程通道
}

func initTimer(fileName string) {
	timeOnce.Do(func() {
		LogName = strings.TrimSuffix(fileName, ".log")
	})
}
...
func (l *TimeWriter) openNew() error {
	...
	if err != nil {
		return fmt.Errorf("can't make directories for new logfile: %s %s", err, l.Dir)
	}
...
func (l *TimeWriter) openExistingOrNew(writeLen int) error {
	...
	//通过curFileName来判断当前写入日志文件是不是要重新创建
